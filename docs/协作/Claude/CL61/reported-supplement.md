...
**2. 四处拟建唯一索引的重复数据 —— 实测全为 0，可以安全建立。**

TL64 末尾四行：`finance_payable 0`、`finance_receivable 0`、`purchase_receipt 0`、`booking_master 0`。
所以首次报告里列为最高风险的
`receipt_payable_source_v1` 与 `ipad v2`，**"因重复数据而失败"这一条风险解除**。
两者其余风险（不幂等、加索引期间的锁）仍在，且仍未实测。
...
3. `stock_take_detail` 行数未知，锁窗口无法估算（首次报告的 Q4 未被 TL64 覆盖）。

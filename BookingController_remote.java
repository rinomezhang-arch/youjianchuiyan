...
public class BookingController {
    ...
    public Result<Map<String, Object>> listWithFilters(
        ...
        try {
            ...
            if (startDate != null && !startDate.isEmpty() && endDate != null && !endDate.isEmpty()) {
                sql.append(" AND bm.booking_date BETWEEN ? AND ?");
    ...
    public ResponseEntity<Result<BookingMaster>> create(@RequestBody Map<String, Object> body) {
        try {
            ...
            if ((tables == null || tables.isEmpty()) && tableIdsRaw != null && !tableIdsRaw.isEmpty()) {
            ...
            if (customerName == null || customerName.isEmpty()) {
                return ResponseEntity.badRequest().body(Result.error(400, "客户姓名不能为空"));
            }
            if (customerPhone == null || customerPhone.isEmpty()) {
                return ResponseEntity.badRequest().body(Result.error(400, "客户手机号不能为空"));
            }
            if (bookingDateObj == null) {
                return ResponseEntity.badRequest().body(Result.error(400, "预订日期不能为空"));
            }
            if ((tables == null || tables.isEmpty()) && (tableIdsRaw == null || tableIdsRaw.isEmpty())) {
                return ResponseEntity.badRequest().body(Result.error(400, "至少选择一个桌台"));
            }

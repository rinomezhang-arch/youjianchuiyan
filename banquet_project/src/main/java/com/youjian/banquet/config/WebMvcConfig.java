 ...
 * 4. ApprovalAuthorityInterceptor：审批批复人白名单，拦截所有 approve/reject 动作
 *
 * 拦截器执行顺序：限流防护（order=-1）→ JWT 全局鉴权（order=0）→ iPad 接口校验（order=1）
 * → 审批批复人白名单（order=2，必须在 JWT 之后，它要用 JWT 解出来的身份）
 */
@Configuration
    ...
    public void addInterceptors(InterceptorRegistry registry) {
        ...
        registry.addInterceptor(jwtAuthInterceptor)
                ...
                .excludePathPatterns(
                        ...
                        // 官网首页招牌菜品预览，访客还没登录。
                        "/api/public/**",
                        // 多 AI 协作台：AI 没有员工账号也拿不到 JWT，走 X-Agent-Token，
                        // 由 AgentCollabController 自己验；其中"点头/驳回"仍单独要求 JWT。
                        "/api/agent/**"
                )
                .order(0);

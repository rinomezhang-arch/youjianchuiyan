...
public class BookingController {
    ...
    public Result<Map<String, Object>> listWithFilters(
        ...
        try {
            // 店长强制查询本店
            storeId = resolveQueryStoreId(storeId);
            // 1. 通过 JPA 加载 booking_master 列表 + 条件过滤（不用复杂子查询，避免各种 SQL 方言语法错误）
            List<BookingMaster> all;
            if (storeId == null) {
                all = bookingMasterRepo.findAll(Sort.by(Sort.Direction.DESC, "createdAt"));
            } else {
                all = bookingMasterRepo.findByStoreIdOrderByBookingDateDesc(storeId);
            }
            List<BookingMaster> filtered = new ArrayList<>(all);
            if (date != null && !date.isEmpty()) {
                LocalDate d = LocalDate.parse(date);
                filtered.removeIf(b -> b.getBookingDate() == null || !d.isEqual(b.getBookingDate()));
            }
            if (time != null && !time.isEmpty()) {
                if (time.contains("午餐")) {
                    filtered.removeIf(b -> b.getBookingTime() == null || !b.getBookingTime().isBefore(LocalTime.of(15, 0)));
            ...
            if (keyword != null && !keyword.isEmpty()) {
                ...
                filtered.removeIf(b ->
                        !((b.getCustomerName() != null && b.getCustomerName().toLowerCase().contains(kw))
                                || (b.getCustomerPhone() != null && b.getCustomerPhone().contains(keyword))));
            }
            if (status != null && !status.isEmpty()) {
                filtered.removeIf(b -> !status.equals(b.getBookingStatus()));
            }
            ...
            List<BookingMaster> pageList = fromIdx >= total ? Collections.emptyList() : filtered.subList(fromIdx, toIdx);

            // 3. 为每页数据查关联 table / dish（一次性批量加载）
            Set<String> bookingIds = pageList.stream().map(BookingMaster::getBookingId).collect(Collectors.toSet());
            Map<String, List<BookingTable>> tablesMap = bookingIds.isEmpty() ? Collections.emptyMap() :
                    bookingTableRepo.findByBookingIdInAndStoreId(bookingIds, storeId)
                            .stream().collect(Collectors.groupingBy(BookingTable::getBookingId));
            Map<String, List<BookingDishDetail>> dishesMap = bookingIds.isEmpty() ? Collections.emptyMap() :
                    bookingDishDetailRepo.findAllById(
                            bookingDishDetailRepo.findAll().stream()
                                    .filter(d -> bookingIds.contains(d.getBookingId()))
                                    .map(BookingDishDetail::getDishBookingId)
                                    .collect(Collectors.toList()))
                            .stream().collect(Collectors.groupingBy(BookingDishDetail::getBookingId));

            List<Map<String, Object>> rows = new ArrayList<>();
            for (BookingMaster bm : pageList) {
                List<BookingTable> bts = tablesMap.getOrDefault(bm.getBookingId(), Collections.emptyList());
                List<BookingDishDetail> bds = dishesMap.getOrDefault(bm.getBookingId(), Collections.emptyList());
                String btNames = bts.stream().map(BookingTable::getTableName).filter(Objects::nonNull).collect(Collectors.joining(", "));
                String dishNames = bds.stream().filter(d -> d.getDishName() != null)
                        .map(d -> d.getDishName() + "×" + (d.getDishQuantity() == null ? 1 : d.getDishQuantity()))
                        .collect(Collectors.joining(", "));

                Map<String, Object> row = bookingMasterToMap(bm);
                row.put("bt_count", bts.size());
                row.put("bt_names", btNames);
                row.put("dish_count", bds.size());
                row.put("dish_names", dishNames);
                rows.add(row);
            }
    ...
    private Map<String, Object> bookingMasterToMap(BookingMaster bm) {
        Map<String, Object> m = new HashMap<>();
        m.put("bookingId", bm.getBookingId());
        m.put("storeId", bm.getStoreId());
        m.put("bookingNo", bm.getBookingNo());
        m.put("bookingDate", bm.getBookingDate());
        m.put("bookingTime", bm.getBookingTime());
        m.put("bookingType", bm.getBookingType());
        m.put("customerId", bm.getCustomerId());
        m.put("customerName", bm.getCustomerName());
        m.put("customerPhone", bm.getCustomerPhone());
        m.put("staffId", bm.getStaffId());
        m.put("staffName", bm.getStaffName());
        m.put("depositAmount", bm.getDepositAmount());
        m.put("guestCount", bm.getGuestCount());
        m.put("tableCount", bm.getTableCount());
        m.put("spareTables", bm.getSpareTables());
        m.put("guestPerTable", bm.getGuestPerTable());
        m.put("bookingStatus", bm.getBookingStatus());
        m.put("status", bm.getStatus());
        m.put("banquetName", bm.getBanquetName());
        m.put("occasionType", bm.getOccasionType());
        m.put("packageName", bm.getPackageName());
        m.put("specialRequest", bm.getSpecialRequest());
        m.put("totalAmount", bm.getTotalAmount());
        m.put("finalAmount", bm.getFinalAmount());
        m.put("paymentStatus", bm.getPaymentStatus());
        m.put("remark", bm.getRemark());
        m.put("createTime", bm.getCreatedAt());
        m.put("updateTime", bm.getUpdatedAt());
        return m;
    }
    ...
    @GetMapping("/stats")
    public Result<Map<String, Object>> getBookingStats(@RequestParam(defaultValue = "1") Long storeId,
                                                       @RequestParam(required = false) String date) {
        try {
            Long effective = resolveQueryStoreId(storeId);
            LocalDate d = (date != null && !date.isEmpty()) ? LocalDate.parse(date) : LocalDate.now();
            String storeFilter = (effective == null) ? "1=1" : "bm.store_id = " + effective;

            // 今日/所选日期 各状态订单数
            String sqlStatus = "SELECT COALESCE(bm.status,'pending') as status, COUNT(*) as cnt FROM booking_master bm " +
                    "WHERE DATE(bm.booking_date) = ? AND " + storeFilter + " GROUP BY bm.status";
            List<Map<String, Object>> byStatus = jdbc.queryForList(sqlStatus, d);
            Map<String, Long> statusMap = new HashMap<>();
            for (Map<String, Object> row : byStatus) {
                String k = (String) row.get("status");
                Long v = ((Number) row.get("cnt")).longValue();
                statusMap.merge(k, v, Long::sum);
            }

            // 本月概览
            String monthStart = String.format("%04d-%02d-01", d.getYear(), d.getMonthValue());
            String monthEnd = d.withDayOfMonth(d.lengthOfMonth()).toString();
            String sqlMonth = "SELECT COUNT(*) as total, COALESCE(SUM(bm.total_amount),0) as revenue " +
                    "FROM booking_master bm WHERE bm.booking_date BETWEEN ? AND ? AND " + storeFilter;
            Map<String, Object> month = jdbc.queryForList(sqlMonth, monthStart, monthEnd).stream().findFirst().orElse(Map.of());

            // 未来7天分布
            String sqlWeek = "SELECT DATE(bm.booking_date) as day, COUNT(*) as cnt FROM booking_master bm " +
                    "WHERE bm.booking_date BETWEEN ? AND ? AND " + storeFilter +
                    " GROUP BY DATE(bm.booking_date) ORDER BY day";
            List<Map<String, Object>> weekly = jdbc.queryForList(sqlWeek, d.toString(), d.plusDays(6).toString());

            Map<String, Object> out = new HashMap<>();
            out.put("date", d.toString());
            out.put("todayTotal", statusMap.values().stream().mapToLong(Long::longValue).sum());
            out.put("byStatus", statusMap);
            out.put("month", month);
            out.put("weekly", weekly);
            return Result.success(out);
        } catch (Exception e) {
            return Result.error(500, "预定统计失败: " + e.getMessage());
        }
    }
    ...
    // ===== 复制预订（GET 兼容：前端部分场景用 GET 预取 / 进页面）=====
    @GetMapping({"/copy", "/swap"})
    public Result<Map<String, Object>> copySwapGetFallback() {
        return Result.success(Map.of("newBookingId", "", "ok", true, "tip", "use POST with body"));
    }

...
public class IpadTableController {
    ...
    public Result<List<Map<String, Object>>> getAllTables(
        ...
        try {
            ...
            List<BookingTable> todayBookingTables = bookingTableRepo.findByStoreIdAndBookingDate(storeId, today);
            List<BookingMaster> todayBookings = bookingRepo.findByStoreIdAndBookingDate(storeId, today);
            Map<String, BookingMaster> bookingMap = todayBookings.stream()
                    .collect(Collectors.toMap(BookingMaster::getBookingId, b -> b, (a, b) -> a));
            // 按 table_id 关联 confirmed/pending 订单
            Map<Integer, BookingTable> tableBookingMap = new HashMap<>();
            for (BookingTable bt : todayBookingTables) {
                BookingMaster bm = bookingMap.get(bt.getBookingId());
                if (bm != null && ("confirmed".equals(bm.getStatus()) || "pending".equals(bm.getStatus()))) {
                    tableBookingMap.putIfAbsent(bt.getTableId(), bt);
                }
            }

            final Map<Integer, BookingTable> finalTableBookingMap = tableBookingMap;
            List<Map<String, Object>> result = tables.stream().map(table -> {
                Map<String, Object> map = new HashMap<>();
                map.put("table_id", table.getTableId());
                map.put("table_number", table.getTableNumber());
                ...
                // 关联今天的订单信息
                BookingTable bt = finalTableBookingMap.get(table.getTableId());
                if (bt != null) {
                    BookingMaster bm = bookingMap.get(bt.getBookingId());
                    if (bm != null) {
                        Map<String, Object> booking = new HashMap<>();
                        booking.put("booking_id", bm.getBookingId());
                        booking.put("customer_name", bm.getCustomerName());
                        booking.put("customer_phone", bm.getCustomerPhone());
                        booking.put("guest_count", bm.getGuestCount());
                        booking.put("booking_type", bm.getBookingType());
                        booking.put("total_amount", bm.getTotalAmount());
                        booking.put("booking_time", bm.getBookingTime());
                        booking.put("booking_date", bm.getBookingDate());
                        booking.put("remark", bm.getRemark());
                        map.put("booking", booking);
                        map.put("table_status", "occupied");
                    }
                }
                return map;
            }).collect(Collectors.toList());

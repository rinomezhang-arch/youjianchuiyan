 ...
 */
@RestController
@RequestMapping({"/api/dict", "/menu-api/dict", "/api/hr/dict"})
public class DictController {
    ...
    /**
     * 按字典编码查询字典项（路径传参：/items/sys_education）
     */
    ...
    public Result<List<Map<String, Object>>> listItems(
        ...
        String sql = """
            ...
            WHERE t.dict_code = ? AND (t.store_id = ? OR t.store_id IS NULL) AND i.is_active = 1
    ...
    /**
     * 按字典编码查询字典项（QueryParam：/items?dictCode=sys_education，方便hr.js调用）
     */
    @GetMapping(value = "/items", params = "dictCode")
    public Result<List<Map<String, Object>>> listItemsByParam(
            @RequestParam String dictCode,
            @RequestParam(defaultValue = "1") Long storeId) {
        return listItems(dictCode, storeId);
    }
    ...
    public Result<List<Map<String, Object>>> listStaff(
        ...
        StringBuilder sql = new StringBuilder(
            "SELECT staff_id, staff_name, staff_no, staff_phone, department, staff_position " +
            "FROM staff_master WHERE store_id = ? AND employment_status = 'active' ");
        List<Object> params = new ArrayList<>();
        params.add(storeId);
        if (keyword != null && !keyword.trim().isEmpty()) {
            sql.append(" AND (staff_name LIKE ? OR staff_no LIKE ? OR staff_phone LIKE ?)");
    ...
    public Result<List<Map<String, Object>>> searchCustomers(
        ...
        StringBuilder sql = new StringBuilder(
            "SELECT customer_id, customer_name, customer_phone, member_level, " +
            "booking_count, total_amount, last_booking_date, remark " +
        ...
        sql.append(" ORDER BY total_amount DESC, customer_name LIMIT ?");
        params.add(limit);
        return Result.success(jdbc.queryForList(sql.toString(), params.toArray()));
    }
}

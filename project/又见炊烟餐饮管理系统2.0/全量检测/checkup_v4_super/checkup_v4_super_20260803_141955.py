...
class Analyzer:
    ...
    def _analyze_foreign_keys(self, tables):
        ...
        for tname in tables:
            ...
            for col in cols:
                ...
                # 动态查找引用表
                ref_table = self._find_referenced_table(col['Field'], tables)
                if not ref_table or ref_table == tname:
                    continue
    ...
    def _find_referenced_table(self, field_name, tables):
        """根据字段名动态推断引用表，无硬编码"""
        base = field_name[:-3]  # 去掉 _id 后缀
        ...
        # 策略2：尝试常见后缀
        suffixes = ['_master', '_info', '_main', '_card', '_order', '_detail', '_config']
        for suffix in suffixes:
            candidate = base + suffix
            if candidate in tables:
                return candidate
        
        # 策略3：尝试前缀匹配（如 store_id → store_xxx）
        matches = [t for t in tables if t.startswith(base + '_')]
        if len(matches) == 1:  # 唯一匹配
            return matches[0]
    ...
    def _analyze_empty_tables(self, tables):
        ...
        for tname, tinfo in tables.items():
            rows = tinfo['rows']
            if rows == 0:
                # 动态判断：表名包含 config/dict/sys/admin/log/history 等关键词视为系统表
                is_sys = self._is_system_table(tname)
                if is_sys:
    ...
    def _is_system_table(self, table_name):
        """动态判断是否为系统/配置表"""
        system_keywords = ['sys_', 'admin_', 'config', 'dict', 'log', 'history', 
                          'ai_', 'session', 'cache', 'audit', 'yield_rate', 'unit_']
        return any(k in table_name for k in system_keywords)
    
    def _analyze_collation(self, tables):
        """字符集一致性检查 - 动态获取数据库默认排序规则"""
        from collections import Counter
        
        # 动态获取数据库默认排序规则
        target_coll = self._get_database_default_collation()
        ...
        for tname, tinfo in tables.items():
            ...
            if coll != target_coll:
                self.add('WARNING', '数据库-数据表', '字符集一致性',
                        ...
                        f'统一为{target_coll}', coll,
                        '不一致会导致JOIN无法使用索引',
                        f"ALTER TABLE `{tname}` CONVERT TO CHARACTER SET utf8mb4 COLLATE {target_coll};")
            else:
                self.add('NORMAL', '数据库-数据表', '字符集一致性',
                        f'{tname} 排序规则正常', target_coll, coll, '')
    
    def _get_database_default_collation(self):
        """动态获取数据库默认排序规则"""
        try:
            result = self.db.query_one("SHOW VARIABLES LIKE 'collation_database'")
            if result:
                return result['Value']
        except:
            pass
    ...
    def _analyze_duplicate_tables(self, tables):
        ...
        # 排除系统表
        system_prefixes = ['sys_', 'admin_', 'config', 'ai_']
        business_tables = [t for t in table_names 
                          if not any(t.startswith(p) for p in system_prefixes)]
        
        # 两两比较相似度
        for a, b in itertools.combinations(business_tables, 2):
            ratio = SequenceMatcher(None, a, b).ratio()
            if ratio >= threshold:
                duplicates.append((a, b, ratio))
        
        if duplicates:
            for a, b, ratio in duplicates:
                self.add('WARNING', '数据库-数据表', '重复业务表',
                        f'疑似重复业务表: {a} 与 {b} (相似度 {ratio:.2f})',
                        '应合并或区分业务含义',
                        f'相似度 {ratio:.2f}',
                        '可能存在冗余表',
                        fix_sql=f"-- 请人工确认是否保留 {a} 或 {b}")
        else:
            self.add('NORMAL', '数据库-数据表', '重复业务表',
                    '未发现明显重复业务表', '无重复表', '无重复表', '')

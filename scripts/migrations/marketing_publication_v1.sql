...
--
-- 【执行要求（必读）】本文件全文件以 `//` 作为语句分隔符：
--   mysql CLI 执行：mysql --delimiter=// <schema> < 本文件
--   Spring ScriptUtils 执行：separator="//"（ScriptUtils.executeSqlScript 的 separator 参数）
--
...
--      （BEFORE UPDATE 复合体触发器条件 SIGNAL + BEFORE DELETE 触发器 SIGNAL，全文件 // 分隔）。
...
--   唯一键、普通索引、外键（子父列序+规则，含 UPDATE_RULE）、触发器
--   （EVENT_MANIPULATION/ACTION_TIMING/ACTION_STATEMENT 关键内容）；任一已存在但定义不符
--   即 SELECT 不存在的哨兵表触发整体失败，绝不带错结构执行任何 DDL（不形成部分迁移）。
--
-- 全文件 // 分隔 + PREPARE/EXECUTE 动态 SQL，兼容 mysql CLI --delimiter=// 与
-- Spring ScriptUtils separator=//；触发器为复合体/单语句体，CREATE TRIGGER IF NOT EXISTS 幂等。
-- ============================================================
...
-- ============================================================
-- 阶段5：发布快照不可改、不可物理删除（复合体/单语句体触发器，全文件 // 分隔，幂等）
...
FOR EACH ROW
BEGIN
  IF NOT (OLD.publication_id <=> NEW.publication_id)

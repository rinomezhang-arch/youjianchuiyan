...
check('B2 未知状态：statusText=状态未知，statusKind=unknown', () => {
  const out = normalizeInquiryLookup({ inquiryNo: 'INQ9004', status: 'weird_future_status', expectedDate: '2026-09-22', partySize: 4, submitTime: '2026-09-14 08:00' })
...
check('C1 成功字段白名单：多余字段（phone/remark/operator/store 数据）被丢弃', () => {
  ...
  assert.deepEqual(Object.keys(out).sort(), ['bookingId', 'expectedDate', 'inquiryNo', 'partySize', 'status', 'statusKind', 'statusText', 'submitTime'])
...
check('D1 手机号校验：11 位 1 开头通过；非法输入被拒', () => {

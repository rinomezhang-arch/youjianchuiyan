...
// 这里只为**客户端 CLI**生成一份删去未知键的临时副本（token 原样保留，身份/权限不变），
...
export function prepareClientConfig() {
  if (cachedPath) return cachedPath
  const srcPath = join(homedir(), '.openclaw', 'openclaw.json')
  const cfg = JSON.parse(readFileSync(srcPath, 'utf8'))
  // 只删触发校验失败的未知键；其余字段（token/url 等）原样保留
  if (cfg?.auth && Object.prototype.hasOwnProperty.call(cfg.auth, 'cooldowns')) {
    delete cfg.auth.cooldowns
  }
  const dir = join(tmpdir(), 'liveness-client-config')

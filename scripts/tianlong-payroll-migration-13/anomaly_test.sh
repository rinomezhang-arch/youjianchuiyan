...
D="co_payroll20_anom1_$(date +%s)_$RANDOM"; newdb "$D"
$MYSQL "$D" -e "
...
D="co_payroll20_anom2_$(date +%s)_$RANDOM"; newdb "$D"
$MYSQL "$D" -e "
CREATE TABLE payroll_payout_record (
  payout_id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
  salary_month VARCHAR(7) NOT NULL, store_id BIGINT NULL, headcount INT NOT NULL,
...
D="co_payroll20_anom3_$(date +%s)_$RANDOM"; newdb "$D"
$MYSQL "$D" -e "
CREATE TABLE payroll_payout_record (
  payout_id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
  salary_month VARCHAR(7) NOT NULL, store_id BIGINT NULL, headcount INT NOT NULL,
...
D="co_payroll20_anom4_$(date +%s)_$RANDOM"; newdb "$D"
$MYSQL "$D" -e "
CREATE TABLE payroll_payout_record (
  payout_id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
  salary_month VARCHAR(7) NOT NULL, store_id BIGINT NULL, headcount INT NOT NULL,
...
D="co_payroll20_anom5_$(date +%s)_$RANDOM"; newdb "$D"
$MYSQL "$D" -e "
CREATE TABLE payroll_payout_record (
  payout_id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
  salary_month VARCHAR(7) NOT NULL, store_id BIGINT NULL, headcount INT NOT NULL,
...
# 反例6：month_salary 七列均已存在但定义错误，且历史值可安全保留 → 原位 MODIFY
D="co_payroll20_anom6_$(date +%s)_$RANDOM"; newdb "$D"
$MYSQL "$D" -e "
ALTER TABLE month_salary
  ADD COLUMN post_salary_snapshot DECIMAL(12,3) NULL,
  ADD COLUMN attendance_pay_snapshot INT NULL,
  ADD COLUMN approved_by VARCHAR(50) NULL,
  ADD COLUMN approved_at DATE NULL,
  ADD COLUMN paid_by TEXT NULL,
  ADD COLUMN paid_at TIMESTAMP NULL,
  ADD COLUMN payout_id INT UNSIGNED NULL;
INSERT INTO staff_master(staff_id,store_id,staff_name) VALUES(101,1,'History Staff');
INSERT INTO month_salary(store_id,staff_id,salary_month,base_salary,gross_salary,net_salary,
  post_salary_snapshot,attendance_pay_snapshot,approved_by,approved_at,paid_by,paid_at,payout_id)
VALUES(1,101,'2026-08',100.00,100.00,100.00,12.340,8,'history-approver','2026-08-31','history-payer','2026-09-01 12:34:56',NULL);" 2>/dev/null
$MYSQL "$D" < "$MIG" 2>/dev/null && ok "反例6 month_salary错误列类型自愈退出0" || bad "反例6 自愈失败"
COL=$($MYSQL "$D" -N -e "SELECT GROUP_CONCAT(CONCAT(column_name,':',column_type,':',is_nullable) ORDER BY FIELD(column_name,'post_salary_snapshot','attendance_pay_snapshot','approved_by','approved_at','paid_by','paid_at','payout_id') SEPARATOR '|') FROM information_schema.columns WHERE table_schema='$D' AND table_name='month_salary' AND column_name IN ('post_salary_snapshot','attendance_pay_snapshot','approved_by','approved_at','paid_by','paid_at','payout_id');" 2>/dev/null)
VAL=$($MYSQL "$D" -N -e "SELECT CONCAT(post_salary_snapshot,'|',attendance_pay_snapshot,'|',approved_by,'|',approved_at,'|',paid_by,'|',paid_at,'|',IFNULL(payout_id,'NULL')) FROM month_salary WHERE staff_id=101 AND salary_month='2026-08';" 2>/dev/null)
EXP="post_salary_snapshot:decimal(10,2):YES|attendance_pay_snapshot:decimal(10,2):YES|approved_by:varchar(40):YES|approved_at:datetime:YES|paid_by:varchar(40):YES|paid_at:datetime:YES|payout_id:bigint:YES"
[ "$COL" = "$EXP" ] && [ "$VAL" = "12.34|8.00|history-approver|2026-08-31 00:00:00|history-payer|2026-09-01 12:34:56|NULL" ] && ok "反例6 七列定义修正且历史值保留" || bad "反例6 列/数据不符($COL/$VAL)"

# 反例7：同名 idx_month_salary_payout 列序错误 → 非唯一 payout_id 索引
D="co_payroll20_anom7_$(date +%s)_$RANDOM"; newdb "$D"
$MYSQL "$D" -e "ALTER TABLE month_salary ADD COLUMN payout_id BIGINT NULL, ADD INDEX idx_month_salary_payout (salary_month);" 2>/dev/null
$MYSQL "$D" < "$MIG" 2>/dev/null && ok "反例7 month_salary同名错误索引自愈退出0" || bad "反例7 自愈失败"
IDX=$($MYSQL "$D" -N -e "SELECT CONCAT(NON_UNIQUE,':',GROUP_CONCAT(COLUMN_NAME ORDER BY SEQ_IN_INDEX SEPARATOR ',')) FROM information_schema.statistics WHERE table_schema='$D' AND table_name='month_salary' AND index_name='idx_month_salary_payout' GROUP BY INDEX_NAME,NON_UNIQUE;" 2>/dev/null)
[ "$IDX" = "1:payout_id" ] && ok "反例7 索引已修正为非唯一 payout_id" || bad "反例7 索引仍错($IDX)"

# 反例8：idx_payout_month 列序正确但误为 UNIQUE → 重建为非唯一索引
D="co_payroll20_anom8_$(date +%s)_$RANDOM"; newdb "$D"
$MYSQL "$D" -e "
CREATE TABLE payroll_payout_record (
  payout_id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
  salary_month VARCHAR(7) NOT NULL, store_id BIGINT NULL, headcount INT NOT NULL,

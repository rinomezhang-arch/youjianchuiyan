param(
    ...
    [ValidateSet(13318)][int]$MysqlPort = 13318,
...
$forbiddenPort = '--port=' + (13300 + 17)
$forbiddenJdbc = 'jdbc:mysql://127.0.0.1:' + (13300 + 17)
foreach ($source in @($PSCommandPath, (Join-Path $PSScriptRoot 'tr24-receipt-browser.mjs'))) {
    $sourceText = Get-Content -LiteralPath $source -Raw
    if ($sourceText.Contains($forbiddenPort) -or $sourceText.Contains($forbiddenJdbc)) {
        throw "Forbidden shared-database endpoint remains in $source; refusing to run"
    }
}
...
if (-not (Get-NetTCPConnection -State Listen -LocalPort $MysqlPort -ErrorAction SilentlyContinue)) { throw "Isolated MySQL $MysqlPort is not listening" }
...
$count = & $mysql --no-defaults --protocol=tcp --host=127.0.0.1 "--port=$MysqlPort" --user=root --batch --skip-column-names --default-character-set=utf8mb4 $schema -e $fixtureSql
...
$names = @('JWT_SECRET','AES_SECRET_KEY','TR24_WEB_PORT','TR24_API_PORT','TR24_MYSQL_PORT','PLAYWRIGHT_MODULE','TR24_WEB_BASE','TR24_SCHEMA','TR24_EVIDENCE','TR29_MANIFEST')
$saved = @{}
...
try {
    ...
    if ([string]::IsNullOrWhiteSpace($backendCommand) -or -not $backendCommand.Contains($expectedDatasource)) {
        throw "Backend datasource does not match isolated target $expectedDatasource"
    }
    Write-Output "Receipt process gate OK: datasource=127.0.0.1:$MysqlPort/$schema"
    ...
    @{task='CO-INTEGRATION-RECEIPT-PORT-34';started=$started;finished=(Get-Date).ToString('o');mysqlPort=$MysqlPort;schema=$schema;driverExit=$driverExit;backendPid=$backendProcess.Id;webPid=$webProcess.Id;manifest=$manifest} |
        ConvertTo-Json | Set-Content -LiteralPath (Join-Path $evidence 'run.json') -Encoding utf8
    Write-Output "TR29_EVIDENCE=$evidence"
}
Write-Output "INTEGRATION_RECEIPT_RUNNER_OK mysql=$MysqlPort schema=$schema"

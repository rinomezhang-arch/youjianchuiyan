<template>
  <div class="tboard">
    ...
    <div class="date-nav">
      ...
      <button class="quick-btn" @click="mvDay(-7)">-7天 · Week</button>
      <button class="quick-btn" @click="mvDay(-30)">-30天 · Mon</button>
      <span class="nav-sep"></span>
    ...
    <transition name="toolbar-slide">
      <div v-show="showActionToolbar" class="inline-action-bar">
        <div class="inline-action-header" @click="toolbarCollapsed = !toolbarCollapsed">
          <span class="collapse-icon">{{ toolbarCollapsed ? '▶' : '▼' }}</span>
          <span v-if="timeType === 'all'" class="action-warning-text"> 全天模式下不可操作，请切换到午餐或晚餐时段 · All-day mode disabled</span>
          <span v-else class="action-summary-text">{{ actionSummary }}</span>
        </div>
        <transition name="toolbar-body">
          <div v-show="!toolbarCollapsed" class="inline-action-body">
            <div v-if="timeType !== 'all'" class="action-btn-row">
...
const showActionToolbar = computed(() => !edit.value && selIds.value.length >= 1 && hasBooked.value)
...
function startSwapMode() {
  console.log('startSwapMode called', {
...
async function loadData() {
  try {
    const res = await getTableBoard({ storeId: 1, date: fmtDate(curDate.value), period: boardPeriod.value })
    if (res.code === 200) {
      const raw = res.data || []
...
.table-item.selected {
  ...
  box-shadow: 0 0 0 4px rgba(124, 58, 237, 0.25), 0 4px 12px rgba(124, 58, 237, 0.2) !important;
  transform: none;
...
.table-item.status-booked.time-lunch {
  background: linear-gradient(145deg, #FFF8E1, #FFF0C8);
  border: 1.5px solid #C9B06B;
}
.table-item.status-booked.time-dinner {
  background: linear-gradient(145deg, #FBE9E7, #F5D5CE);
  border: 1.5px solid #C4917A;
}
.table-item.status-booked.time-both {
  background: linear-gradient(145deg, #FFFDE7, #FFF8C4);
  border: 1.5px solid #C9B85C;
}
...
.inline-action-header:hover {
  background: linear-gradient(135deg, rgba(196, 163, 90, 0.1) 0%, rgba(196, 163, 90, 0.04) 100%);
}
.collapse-icon {
  font-size: 11px;
...
.toolbar-body-enter-active,
.toolbar-body-leave-active {
  transition: all 0.25s cubic-bezier(0.4, 0, 0.2, 1);
}
.toolbar-body-enter-from,
.toolbar-body-leave-to {
  opacity: 0;
  max-height: 0;
  padding-top: 0;
  padding-bottom: 0;

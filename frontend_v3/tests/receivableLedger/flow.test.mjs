...
// 真实后端回执只含主键，"自相矛盾"只体现在 receipt 主键被改成无效值时 validate 判 false。
// 改 payload.totalAmount 不会触发锁死——buildPayload 重建后仍与盘上一致，回执也不含金额可比对。
test('盘上 receipt 主键被改坏时锁死，不拿去重发', async () => {
  ...
  saved.receipt.receivableId = 0          // 主键被改成无效值，validate 会判 false
...
// 真实后端回执只含主键，"对不上"只体现在主键缺失或非正整数时 validate 判 false。
// 改 no 字段不影响判断——前端不校验 no，单号由 requestId 推导，列表核对时才用。
test('回执主键无效时当作结果未知，保留恢复记录', async () => {
  const api = createSyntheticApi()
  const storage = memoryStorage()
  const broken = {
    ...api,
    createReceivable: async body => {
      ...
      return { ...real, data: { receivableId: 0 } }   // 主键无效：服务端没正常落库或回执损坏
    }
  }

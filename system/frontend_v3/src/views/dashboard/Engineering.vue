...
// --- API 函数 ---
const getMaintenanceOrders = () => request({ url: '/engineering/work-orders', method: 'get' })
const getEnergySummary = () => request({ url: '/energy/monthly-summary', method: 'get' })
const getEnergyTrend = () => request({ url: '/energy/trend', method: 'get' })
const getSafetyIssues = () => request({ url: '/engineering/inspections', method: 'get', params: { type: 'safety' } })
...
onMounted(async () => {
  loading.value = true
  try {
    const [maintenanceRes, summaryRes, trendRes, safetyRes] = await Promise.all([
      getMaintenanceOrders(),
      getEnergySummary(),
      getEnergyTrend(),
      getSafetyIssues(),
    ])
    const orders = maintenanceRes.data || []
    const summary = summaryRes.data || {}
    const inspections = safetyRes.data || []
    maintenanceOrders.value = orders.map(item => ({
      id: item.id,
      title: item.title,
    ...
    energyTrend.value = trendRes.data || []
    safetyIssues.value = inspections.map(item => ({
      id: item.id,
      title: item.title,
      location: item.remark || '未填写位置',
      time: item.created_at?.replace('T', ' ') || '',
      severity: item.status === 'pending' ? 'high' : 'low',
      status: item.status === 'completed' ? 'resolved' : item.status
    }))
    stats.value = {
      ...stats.value,
      maintenanceTasks: orders.length,
      maintenancePending: orders.filter(item => item.status === 'pending').length,
      maintenanceDone: orders.filter(item => item.status === 'completed').length,
      energyUsage: `¥${(Number(summary.electric?.cost || 0) + Number(summary.water?.cost || 0) + Number(summary.gas?.cost || 0)).toFixed(2)}`,
      energyElectric: Number(summary.electric?.usage || 0),
      energyWater: Number(summary.water?.usage || 0),
      safetyIssues: inspections.length,
      safetyPending: inspections.filter(item => item.status === 'pending').length,
      safetyResolved: inspections.filter(item => item.status === 'completed').length
    }
  } catch (e) {

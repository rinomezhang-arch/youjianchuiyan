...
// API functions
const getEnergyCurrentMonth = () => request.get('/energy/monthly-summary')
const getEnergyMonthlyTrend = () => request.get('/energy/trend')
const getEnergyReadings = (params) => request.get('/energy/records', { params })
const createEnergyReading = (data) => request.post('/energy/records', data)
...
const fetchData = async () => {
  ...
  try {
    ...
    if (monthRes.data) {
      ...
      currentMonth.value = {
        electric: Number(summary.electric?.usage || 0), electricChange: 0,
        water: Number(summary.water?.usage || 0), waterChange: 0,
        gas: Number(summary.gas?.usage || 0), gasChange: 0,
        cost: Number(summary.electric?.cost || 0) + Number(summary.water?.cost || 0) + Number(summary.gas?.cost || 0)
      }
    }
    if (trendRes.data) monthlyData.value = trendRes.data
    const rows = Array.isArray(readingsRes.data) ? readingsRes.data : []
    const byDate = new Map()
    rows.forEach(row => {
      const date = row.record_date || row.recordDate
      const item = byDate.get(date) || { id: date, date, electricMeter: 0, waterMeter: 0, gasMeter: 0, dailyElectric: 0, dailyWater: 0, dailyCost: 0, recorder: row.recorder || '' }
      const type = row.energy_type || row.energyType
      if (type === 'electric') { item.electricMeter = Number(row.meter_reading || 0); item.dailyElectric = Number(row.daily_usage || 0) }
      if (type === 'water') { item.waterMeter = Number(row.meter_reading || 0); item.dailyWater = Number(row.daily_usage || 0) }
      if (type === 'gas') item.gasMeter = Number(row.meter_reading || 0)
      item.dailyCost += Number(row.daily_cost || 0)
      byDate.set(date, item)
    })
    readings.value = [...byDate.values()]
  } catch (e) {
...
const saveReading = async () => {
  ...
  try {
    loading.value = true
    await Promise.all([
      createEnergyReading({ recordDate: form.value.date, energyType: 'electric', meterReading: form.value.electricMeter, dailyUsage: 0, dailyCost: 0, recorder: form.value.recorder, remark: '页面录入' }),
      createEnergyReading({ recordDate: form.value.date, energyType: 'water', meterReading: form.value.waterMeter, dailyUsage: 0, dailyCost: 0, recorder: form.value.recorder, remark: '页面录入' }),
      createEnergyReading({ recordDate: form.value.date, energyType: 'gas', meterReading: form.value.gasMeter, dailyUsage: 0, dailyCost: 0, recorder: form.value.recorder, remark: '页面录入' })
    ])

...
async function fetchBills() {
  loading.value = true
  try {
    const res = await request.get('/bookings', { params: { pageSize: 100 } })
    if (res.data) {
      const rows = res.data.content || res.data || []
      bills.value = rows.map(booking => ({
        ...booking,
        billNo: booking.billNo || booking.bookingNo || booking.bookingId,
        tableName: booking.tableName || booking.roomName || '-',
        guestCount: booking.guestCount || booking.peopleCount || 0,
        dishCount: booking.dishCount || booking.bookingDishCount || 0,
        totalAmount: Number(booking.totalAmount || booking.payableAmount || 0),
        payAmount: Number(booking.payAmount || booking.paidAmount || 0),
        payMethod: booking.payMethod || booking.paymentMethod,
        status: booking.billStatus || (booking.paymentStatus === 'paid' ? 'settled' : 'unsettled'),
        settledAt: booking.settledAt || booking.paymentTime,
        operator: booking.operator || booking.createdBy,
        openTime: booking.openTime || booking.createdAt,
        dishes: booking.dishes || []
      }))
      const settled = bills.value.filter(bill => bill.status === 'settled')
      stats.value = {
        todayCount: bills.value.length,
        todayRevenue: settled.reduce((sum, bill) => sum + bill.payAmount, 0),
        unsettledCount: bills.value.filter(bill => bill.status === 'unsettled').length,
        todayRefund: bills.value.filter(bill => bill.status === 'refunded').reduce((sum, bill) => sum + bill.payAmount, 0),
        refundCount: bills.value.filter(bill => bill.status === 'refunded').length
      }
    }
  } catch (error) {
    bills.value = []
    stats.value = { todayCount: 0, todayRevenue: 0, unsettledCount: 0, todayRefund: 0, refundCount: 0 }
    ElMessage.error(errorMessage(error, '账单加载失败'))
  } finally {
    loading.value = false
  }
}

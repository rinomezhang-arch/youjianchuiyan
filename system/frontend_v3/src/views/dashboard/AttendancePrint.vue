...
async function fetchData() {
  try {
    const [attendanceRes, staffRes] = await Promise.all([
      request.get('/hr/attendance', { params: { storeId: 1 } }),
      request.get('/hr/staff', { params: { storeId: 1 } })
    ])
    const monthPrefix = `${selectedMonth.value}-`
    const staffById = new Map((staffRes.data || []).map(staff => [Number(staff.staffId), staff]))
    const grouped = new Map()

    for (const item of attendanceRes.data || []) {
      if (!String(item.attendanceDate || '').startsWith(monthPrefix)) continue
      const staff = staffById.get(Number(item.staffId)) || {}
      const department = staff.departmentName || staff.department || '未分配'
      if (selectedDept.value && department !== selectedDept.value) continue
      if (!grouped.has(item.staffId)) {
        grouped.set(item.staffId, {
          name: staff.staffName || staff.name || `员工 ${item.staffId}`,
          department,
          records: Array(daysInMonth.value).fill(''),
          summary: { attendance: 0, rest: 0, leave: 0, overtime: 0, compLeave: 0 }
        })
      }
      const row = grouped.get(item.staffId)
      const day = dayjs(item.attendanceDate).date() - 1
      const status = item.absent ? 'absent' : item.lateMinutes > 0 ? 'late' : item.earlyLeaveMinutes > 0 ? 'early' : (item.status || 'present')
      row.records[day] = status
      if (status === 'leave') row.summary.leave += 1
      else if (status !== 'absent') row.summary.attendance += 1
    }

    attendanceData.value = [...grouped.values()]
    departments.value = [...new Set((staffRes.data || []).map(staff => staff.departmentName || staff.department).filter(Boolean))]
  } catch (error) {
    attendanceData.value = []
    departments.value = []
    ElMessage.error(errorMessage(error, '获取考勤数据失败'))
  }
}

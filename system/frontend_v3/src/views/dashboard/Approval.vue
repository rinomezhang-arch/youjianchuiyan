...
async function refreshData() {
  ...
  try {
    ...
    const data = res.data || []
    const typeMap = { leave: '请假', overtime: '加班', purchase: '采购', expense: '费用', stock_loss: '报损' }
    const statusMap = { pending: '待审批', approved: '已通过', rejected: '已驳回', cancelled: '已取消' }
    approvalList.value = data.map(item => ({
      ...item,
      approvalNo: item.flowNo,
      dept: typeMap[item.flowType] === '请假' || typeMap[item.flowType] === '加班' ? '人事' : '',
      type: typeMap[item.flowType] || item.flowType,
      applicant: item.applicantName || '-',
      applyDate: item.createdTime?.slice(0, 10) || '-',

<template>
  <div class="cost-page">
    ...
    <div class="loop-strip" v-loading="loopLoading">
      <div class="loop-stat"><span>菜品档案</span><strong>{{ loopStats.dishes || 0 }}</strong></div>
      <div class="loop-stat is-ready"><span>成本已就绪</span><strong>{{ loopStats.costReadyDishes || 0 }}</strong></div>
      <div class="loop-stat is-warning"><span>待处理配方草稿</span><strong>{{ loopStats.draftRecipes || 0 }}</strong></div>
      <div class="loop-copy">只有成本卡审批通过的菜品，才能进入菜单与采购需求展开。</div>
    </div>
    <div class="cost-toolbar">
      <el-button type="primary" @click="openNew">成本录入</el-button>
      <el-button size="small" @click="fetchLoopStats">刷新门禁</el-button>
...
async function fetchLoopStats() {
  ...
  try {
    const res = await request.get('/cost-procurement-loop/dashboard')
...
onMounted(async () => {
  ...
  error.value = ''
  try {
    await Promise.all([fetchData(), fetchAllIngredients(), fetchLoopStats()])
  } catch (e) {

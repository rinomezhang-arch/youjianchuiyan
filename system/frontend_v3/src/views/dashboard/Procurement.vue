<template>
  <div class="page">
    ...
    <div class="loop-strip" v-loading="loopLoading">
      <div class="loop-stat is-warning"><span>经理待审</span><strong>{{ loopStats.pendingManagerApproval || 0 }}</strong></div>
      <div class="loop-stat is-danger"><span>总经理待审</span><strong>{{ loopStats.pendingGmApproval || 0 }}</strong></div>
      <div class="loop-stat is-ready"><span>待分批验收</span><strong>{{ loopStats.pendingReceipt || 0 }}</strong></div>
      <div class="loop-copy">需求缺口生成申请，依次完成经理、总经理审批后才允许验收入库。</div>
    </div>
...
.loop-stat.is-danger strong { color:#b91c1c; }

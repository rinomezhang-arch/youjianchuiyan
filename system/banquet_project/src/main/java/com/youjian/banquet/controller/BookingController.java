...
public class BookingController {
    ...
    public Result<Map<String, Object>> stats(
        ...
        Map<String, Object> row = jdbc.queryForMap(
                "SELECT "
                        + "SUM(CASE WHEN booking_status='confirmed' THEN 1 ELSE 0 END) AS confirmedCount, "
                        + "COALESCE(SUM(guest_count),0) AS totalPeople, "
                        + "SUM(CASE WHEN booking_time < '15:00:00' THEN 1 ELSE 0 END) AS lunchCount, "
                        + "SUM(CASE WHEN booking_time >= '15:00:00' THEN 1 ELSE 0 END) AS dinnerCount "
                        + "FROM booking_master WHERE store_id=? AND booking_date=?",
                storeId, java.sql.Date.valueOf(targetDate));

...
public class HRController {
    ...
    public Result<LeaveRecord> approveLeave(@PathVariable Integer id, @RequestBody Map<String, Object> body) {
        try {
            LeaveRecord existing = leaveRepo.findById(id)
                    .orElseThrow(() -> new IllegalArgumentException("请假记录不存在"));
            assertWritableStore(existing.getStoreId());
            String status = body.get("status") == null ? "approved" : body.get("status").toString();
            String remark = body.get("remark") == null ? null : body.get("remark").toString();
            ApprovalFlow flow = approvalService.findByBusiness("leave", id.longValue()).orElse(null);
            if (flow != null && "pending".equals(flow.getStatus())) {
                if ("rejected".equals(status)) approvalService.reject(flow.getId(), remark);
                else approvalService.approve(flow.getId(), remark);
                return Result.success(leaveRepo.findById(id).orElse(existing));
            }
            existing.setStatus(status);
            existing.setApproverId(getCurrentStaffId().intValue());
    ...
    public Result<Overtime> updateOvertime(@PathVariable Integer id, @RequestBody Map<String, Object> body) {
        try {
            Overtime existing = overtimeRepo.findById(id)
                    .orElseThrow(() -> new IllegalArgumentException("加班记录不存在"));
            assertWritableStore(existing.getStoreId());
            if (body.get("staffId") != null) existing.setStaffId(Integer.valueOf(body.get("staffId").toString()));
            if (body.get("overtimeDate") != null) existing.setOvertimeDate(java.time.LocalDate.parse(body.get("overtimeDate").toString()));
            java.time.LocalDate date = existing.getOvertimeDate() == null ? java.time.LocalDate.now() : existing.getOvertimeDate();
            if (body.containsKey("startTime")) existing.setStartTime(parseDateTime(body.get("startTime"), date));
            if (body.containsKey("endTime")) existing.setEndTime(parseDateTime(body.get("endTime"), date));
    ...
    public Result<Overtime> approveOvertime(@PathVariable Integer id, @RequestBody Map<String, Object> body) {
        try {
            ...
            String status = body.get("status") == null ? "approved" : body.get("status").toString();
            String remark = body.get("remark") == null ? null : body.get("remark").toString();
            ApprovalFlow flow = approvalService.findByBusiness("overtime", id.longValue()).orElse(null);
            if (flow != null && "pending".equals(flow.getStatus())) {
                if ("rejected".equals(status)) approvalService.reject(flow.getId(), remark);
                else approvalService.approve(flow.getId(), remark);
                return Result.success(overtimeRepo.findById(id).orElse(existing));
            }
            existing.setStatus(status);
            existing.setApproverId(getCurrentStaffId().intValue());
            existing.setApproveTime(java.time.LocalDateTime.now());
    ...
    private void assertWritableStore(Long recordStoreId) {
        Long requiredStoreId = resolveWriteStoreId();
        if (requiredStoreId != null && !requiredStoreId.equals(recordStoreId)) {
            throw new SecurityException("无权操作其他门店的人事记录");
        }
    }

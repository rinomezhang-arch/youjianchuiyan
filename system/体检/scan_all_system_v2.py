...
var doFilter = window.doFilter = function(){
 ...
 allData.forEach(function(item){
  ...
  if (!showDash && currentLevel === "all" && item.module === "数据看板") return;
  if (mf === "__DASH__" && item.module !== "数据看板") return;
 ...
 document.getElementById("resultCount").textContent = "显示 " + cnt + " 条结果";
};
...
def main():
    ...
    try:
        ...
        report = ReportGenerator([
            ...
            scanner_back, scanner_bsec, scanner_front, scanner_fsec, scanner_apideep, scanner_api, scanner_cfg, scanner_biz,
